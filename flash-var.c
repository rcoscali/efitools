...
main(int argc, char *argv[])
{
  ...
  if (!buf)
    {
      fprintf(stderr, "Memory allocation failed when reading flash\n");
      goto exit_error_1;
    }
  
  for (i = 0; ; i += chunk)
    {
      (void)lseek(flashfile, i, SEEK_SET);
  ...
  (void)lseek(flashfile, offset, SEEK_SET);
  free(buf);
  buf = malloc(sizeof(VARIABLE_STORE_HEADER));
  if (!buf)
    {
      fprintf(stderr, "Memory allocation failed when reading variable store header\n");
      goto exit_error_1;
    }
  (void)read(flashfile, buf, sizeof(VARIABLE_STORE_HEADER));
  ...
  buf = (void *)(vh + 1);
  (void)memcpy (buf, var, varlen);
  buf += varlen;
  (void)memcpy (buf, vardata, varfilesize);
  lseek(flashfile, offset, SEEK_SET);
  (void)write(flashfile, vsh, vsh->Size);
  close(flashfile);
  free(buf);
