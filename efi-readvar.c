...
main(int argc, char *argv[])
{
  ...
  int i, found = 1, sig = -1, entry = -1, fd = -1;
      ...
      if (fd < 0)
        {
          ...
          errmsg = (char *)malloc(strlen("failed to open %s: ") + strlen(file) + 1);
          if (!errmsg)
            {
          ...
          sprintf(errmsg, "failed to open %s: ", file);
  ...
  for (i = 0; i < ARRAY_SIZE(owners); i++)
    {
      ...
      if (file)
        {
          int wrtbytes = write(fd, buf, len);
          if (wrtbytes != len)
            {
              fprintf(stderr, "warning : didn't wrote (%d) as much bytes as requested (%d)", wrtbytes, len);
            }
        }
      else
